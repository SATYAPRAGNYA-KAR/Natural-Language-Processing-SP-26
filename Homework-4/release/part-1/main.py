...
def create_augmented_dataloader(args, dataset):
    ...
    small_transformed=train_dataset.shuffle(seed=42).select(range(5000))
    small_transformed=small_transformed.map(custom_transform, load_from_cache_file=False)
    augmented_dataset=datasets.concatenate_datasets([train_dataset, small_transformed])
